...
class Cluster:
    ...
    def create_cluster(self, features):
        ...
        try:
            ...
            self.logger.log_training_pipeline('TRAINING PROCESS: CLUSTERING : Creating Clusters [algorithm = k-means++]')
            model = KMeans(n_clusters=k_clusters, init='k-means++')
            cluster_label = model.fit_predict(features)
            self.logger.log_training_pipeline('TRAINING PROCESS: CLUSTERING: Saving Model to Cloud')
    ...
    def knee_finder(self, features):
        ...
        """
        try:
            self.logger.log_training_pipeline('TRAINING PROCESS: CLUSTERING: Finding Optimal number of Clusters')
            wcss = []
            ...
            self.logger.log_training_pipeline(
                'TRAINING PROCESS: CLUSTERING: Optimal Number of Clusters Found : [{}]'.format(self.k_clusters)
            )
